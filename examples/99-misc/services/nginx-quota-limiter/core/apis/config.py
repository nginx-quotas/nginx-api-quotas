"""Business Logic for Quota Limit Policy Configuration API in Data Plane"""
...
class QuotaLimitConfig:
    ...
    def put(self, data, user_id=None):
        key = Level.GROUP if user_id is None else user_id
    ...
    def _upsert(self, key, data):
        ...
        if key == Level.GROUP:
            self.limiter.configure_group_limit(rps=quota_limit)
        else:
