"""Token Bucket for Quota Limiter"""

import time


class QuotaLimitBucket:
    """Quota Limit Bucket to manage quota remaining for each key

    This bucket is created and managed per each key such as user-id or group
    system.
    ...
    def quota_limit(self):
        ...
        which can be changed by any admin via distributed quota-limiter apps."""
        return self._quota_limit

    def quota_remaining(self):
        """This is to return quota remainining for the time right after the
           process_request() is called in the quota-limiter.."""
